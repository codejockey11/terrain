...
int main(int ac, char* argv[])
{
	...
	pixelStart = (bmp->pixels + (size_t)width * height * bmp->bytesPerPixel) - width;
	...
	for (int h = 0; h < height; h++)
	{
		...
		for (int w = 0; w < width; w++)
		{
			...
			v->p.y = (float)(bmp->paletteentry[*pixelStart].peRed * vheight);
	...
	printf("Writing Tiles And Clips\n");
	...
	err = fopen_s(&clips, "clips.txt", "wb");

	if (err)
	{
		printf("\nError opening clips.txt wb\n");
	...
	if (average)
	{
		...
		for (int h = 0; h < height - 1; h++)
		{
			for (int w = 0; w < width - 1; w++)
			{
				...
				float sumX = t1.f.n.x + t2.c.n.x + t2.d.n.x + t3.a.n.x + t4.e.n.x + t4.b.n.x;
				float sumY = t1.f.n.y + t2.c.n.y + t2.d.n.y + t3.a.n.y + t4.e.n.y + t4.b.n.y;
				float sumZ = t1.f.n.z + t2.c.n.z + t2.d.n.z + t3.a.n.z + t4.e.n.z + t4.b.n.z;
				...
				t1.f.n.x = t2.c.n.x = t2.d.n.x = t3.a.n.x = t4.e.n.x = t4.b.n.x = nvn.p.x;
				t1.f.n.y = t2.c.n.y = t2.d.n.y = t3.a.n.y = t4.e.n.y = t4.b.n.y = nvn.p.y;
				t1.f.n.z = t2.c.n.z = t2.d.n.z = t3.a.n.z = t4.e.n.z = t4.b.n.z = nvn.p.z;
	...
	if (vertices)
	{
		...
		for (int h = 0; h < height - 1; h++)
		{
			for (int w = 0; w < width - 1; w++)
			{
				...
				fwrite(&t1.a, sizeof(CVertexNT), 1, verticesFile);

				c++;
			}

			fwrite(&t1.c, sizeof(CVertexNT), 1, verticesFile);
		...
		for (int w = 0; w < width - 1; w++)
		{
			...
			fwrite(&t1.b, sizeof(CVertexNT), 1, verticesFile);

			c++;
		}

		fwrite(&t1.f, sizeof(CVertexNT), 1, verticesFile);
	...
	if (1)
	{
		...
		for (int h = 0; h < height - 1; h++)
		{
			for (int w = 0; w < width - 1; w++)
			{
				...
				CVertex n = CVertex(t1.a.n.x, t1.a.n.y, t1.a.n.z);
...
void WriteTile(CVertex* a, CVertex* b, CVertex* c, CVertex* n1, CVertex* d, CVertex* e, CVertex* f, CVertex* n2, BOOL uuv, float u, float v)
{
	...
	fwrite(a, sizeof(CVertex), 1, clips);
	fwrite(b, sizeof(CVertex), 1, clips);
	fwrite(c, sizeof(CVertex), 1, clips);

	fwrite(n1, sizeof(CVertex), 1, clips);

	fwrite(d, sizeof(CVertex), 1, clips);
	fwrite(e, sizeof(CVertex), 1, clips);
	fwrite(f, sizeof(CVertex), 1, clips);

	fwrite(n2, sizeof(CVertex), 1, clips);
}
